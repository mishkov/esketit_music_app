...
Use-case models must stay independent from transport details. Do not put REST field names, JSON keys, endpoint-specific enums, header names, status-code handling, or request/response serialization rules in this layer. If backend uses values like `my_vibe`, that mapping belongs in `Esketit Rest Api`, while `Use Case` should keep domain-safe names like `myVibe`.
...
This layer is responsible for translating between backend contracts and app models. JSON keys, query params, path params, request bodies, response parsing, and mapping between API enum values and use-case/domain enums must be implemented here, not in `Use Case` or `UI`.
...
Practical rule: if a string/value exists only because one backend endpoint expects or returns it, keep it out of `Domain` and `Use-case`. Convert it at the `Esketit Rest Api` boundary.
